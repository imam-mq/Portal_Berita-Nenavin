	...
	<body class="font-[Poppins]">
		...
		<nav id="Category" class="max-w-[1130px] mx-auto flex justify-center items-center mt-[30px] px-4">
			<div class="category-container flex overflow-x-auto space-x-4 px-4 py-2 scrollbar-hide" 
				...
				<a href="{{ route('front.category', $item_category->slug) }}" 
					...
					<div class="w-6 h-6 flex shrink-0">
						<img src="{{ Storage::url($item_category->icon) }}" alt="icon" class="w-6 h-6 object-contain">
					</div>
					<span class="text-sm md:text-base">{{ $item_category->name }}</span>
				</a>
				@endforeach
			</div>
		</nav>
		...
		<section id="Article-container" class="max-w-[1130px] mx-auto flex flex-col lg:flex-row gap-10 lg:gap-20 mt-[50px] px-4">
			...
			<aside class="side-bar flex flex-col w-full lg:w-[300px] shrink-0 gap-10">
				...
				<div class="ads flex flex-col gap-3 w-full">
					<a href="{{$square_ads_1->link}}" class="block rounded-lg overflow-hidden shadow-lg transition-all hover:shadow-xl">
						<img src="{{Storage::url($square_ads_1->thumbnail)}}" class="object-cover w-full h-auto rounded-lg" alt="ads" />
					</a>
				...
				<div class="ads flex flex-col gap-3 w-full">
					@if($square_ads_2)
						<a href="{{ $square_ads_2->link }}" class="block rounded-lg overflow-hidden shadow-lg transition-all hover:shadow-xl">
							<img src="{{ Storage::url($square_ads_2->thumbnail) }}" class="object-cover w-full h-auto rounded-lg" alt="ads" />
						</a>
					@else
					...
					<p class="font-medium text-sm leading-[21px] text-[#A3A6AE] flex gap-1 items-center">
						Our Advertisement 
						<a href="{{$bannerads->link}}" class="w-[18px] h-[18px]">
							<img src="{{asset('assets/images/icons/message-question.svg')}}" alt="icon" />
						</a>
					</p>
				</div>
		...
		<section id="Advertisement" class="max-w-[1130px] mx-auto flex justify-center mt-[70px] px-4">
			<div class="flex flex-col gap-3 w-full max-w-[900px]">
				<a href="{{$bannerads->link}}" class="block rounded-2xl overflow-hidden shadow-lg transition-all hover:shadow-xl">
		...
		<section id="Up-to-date" class="w-full flex justify-center mt-[70px] py-[50px] bg-[#F9F9FC] px-4">
			<div class="max-w-[1130px] mx-auto flex flex-col gap-[30px]">
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[20px] md:gap-[30px]">
					...
					<a href="{{route('front.details', $article->slug)}}" class="group">
						<div
